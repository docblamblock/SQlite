...
if ($_GET['lastUpdate']=="")
   $lastUpdate = "2012-03-01 00:00:00";  // will ensure all tourist info data is pulled from dB 
else   
  $lastUpdate = date("Y-m-d H:i:s", $_GET['lastUpdate']);   // time the app calling PROC was last updated


$query = "SELECT infoID, venue, address, gps, location, category, sub_cat, short_info, info, short_info_ga_IE, info_ga_IE, features, web, open, youtube, contact, img, added, active, position, distance_from_city_centre  FROM tourist_info where active=? AND added>=? order by category, infoID limit $limit"; 
...
if ($stmt = $mysqli->prepare($query)) {
    ...
    // bind the parameters
    $stmt->bind_param("is", $one, $lastUpdate);
